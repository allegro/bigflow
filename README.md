...
With `deploy-dags` command you can upload DAG files generated by `dagbuilder`
to your Composer's DAGs Folder, which is underlied by a Google Cloud Storage bucket.
...
Upload DAG files from local project dir using local authentication
and `Config` object defined in local `deployment_config.py`:
...
Choose environment name from `Config` object defined in `deployment_config.py`: 
 
```bash 
bgq deploy-dags --env dev
bgq deploy-dags --env prod
```

In this scenario, required props are read from `deployment_config.py`, which should 
contain an instance of `Config` declared as `deployment_config`. For example:
...
Upload DAG files from given project dir using service account authentication
(without deployment_config.py):
...
Choose from two authentication methods `local_account` (default):

```bash
bgq deploy-dags --auth-method=local_account
```  
  
or `service_account` authentication with Vault:
